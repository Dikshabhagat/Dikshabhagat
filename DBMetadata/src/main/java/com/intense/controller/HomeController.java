...
public class HomeController {
	 @Autowired private MyRepository myRepository;
	  ...
	  @Autowired private ColumnsRepository columnsRepository;
...
//				});
	 ...
	 * @Autowired private ColumnsRepository columnsRepository;
	 * 
	 * @GetMapping("/")//method1 public ResponseEntity<MyResponse> welcome() {
	 * return new ResponseEntity(new
	 * MyResponse("default path","welcome to my DB Metadata API",null),HttpStatus.OK
	 * ); }
	 * 
	 * @GetMapping("/tables")//call method of repost and the list of my table public
	 * ResponseEntity<List<MyTable>> getAllTables() throws Exception { MyResponse
	 * myResponse=new MyResponse("list of all tables and columns",
	 * "list of all tables and columns in the database",
	 * myRepository.getAllTables()); return new
	 * ResponseEntity(myResponse,HttpStatus.OK);// }
	 * 
	 * @PostMapping("/tables")// get the data public ResponseEntity<List<MyTable>>
	 * saveAllTables(@RequestBody List<MyTable> tableList) //send the data in the
	 * body { tableList.stream().forEach( (table)->{List<Columns_Fields>
	 * columnList=table.getColumns(); columnList.stream().forEach(
	 * (column)->{columnsRepository.save(column);}); myTableRepository.save(table);
	 * });
	 * 
	 * MyResponse myResponse=new
	 * MyResponse("all tables and columns saved successfully",
	 * "as response list of all tables and columns in the database", tableList);
	 * return new ResponseEntity(myResponse,HttpStatus.OK); }
	 */
}
