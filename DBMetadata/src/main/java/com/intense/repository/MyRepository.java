...
public class MyRepository {
	...
	public List<MyTable> getAllTables() throws Exception { // coz we have 

		List<MyTable> tableList=new ArrayList<MyTable>();
		Connection connection = dataSource.getConnection();//got connectn 
		
		DatabaseMetaData metaData = connection.getMetaData();//hold all the information for about db  
		String[] types = { "TABLE" };

		ResultSet rs = metaData.getTables(null, null, "%", types);//obj that will get the list of every table
		...
		while (rs.next()) {
			...
			String tableName = rs.getString(3);
			 ResultSet columns = metaData.getColumns(null, null, tableName, null);//colm inside that table nested while loop
